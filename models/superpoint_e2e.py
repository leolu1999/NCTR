...
def top_k_keypoints_dev4(keypoints, scores, k: int):
    if k >= len(keypoints):
        if len(keypoints) % 4 == 0:
            return keypoints, scores
        else:
            k = len(keypoints) - len(keypoints) % 4
            scores, indices = torch.topk(scores, k, dim=0)
            return keypoints[indices], scores
    scores, indices = torch.topk(scores, k, dim=0)
    return keypoints[indices], scores
...
class SuperPoint(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.config['weights_path']:
            ...
            if 'model_sp' in weights:
                load_dict = weights['model_sp']
                self.load_state_dict(load_dict)
                print('Loaded trained SuperPoint model (\"{}\" weights)'.format(self.config['weights_path']))
            else:
    ...
    def forward(self, data, curr_max_kp=None, curr_key_thresh=None):
        ...
        if curr_max_kp >= 0:
            keypoints, scores = list(zip(*[
                top_k_keypoints_dev4(k, s, curr_max_kp)
                for k, s in zip(keypoints, scores)]))
    ...
    def forward_train(self, data):
        ...
        for i, (k, s) in enumerate(zip(keypoints, scores)):
            ...
            if len(k) < self.config['max_keypoints']:
                ...
                scores[i] = torch.cat([scores[i], torch.zeros(to_add_points, dtype=torch.float32, device=s.device)*0.1], dim=0)
