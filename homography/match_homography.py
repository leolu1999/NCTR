...
from utils.common import (make_matching_plot, AverageTimer, pose_auc, read_image_with_homography, compute_pixel_error)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Image pair matching and pose evaluation with NCTR',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument(
        '--input_homography', type=str, default='txt/R1M_test_images.txt',
        help='Path to the list of image pairs and corresponding homographies')
    parser.add_argument(
        '--input_dir', type=str, default='Z:\R1M_dataset\\test',
    ...
    parser.add_argument(
        '--NCTR', help='NCTR weights')
    parser.add_argument(
        '--superpoint', default='models\weights\superpoint_v1.pth',
    ...
    # Load the SuperPoint and NCTR models.
    device = 'cuda' if torch.cuda.is_available() and not opt.force_cpu else 'cpu'
    print('Running inference on device \"{}\"'.format(device))
    if os.path.isfile(opt.NCTR) and (os.path.splitext(opt.NCTR)[-1] in ['.pt', '.pth']):
        curr_weights_path = str(opt.NCTR)
    else:
        raise ValueError("Given --NCTR path doesn't exist or invalid")
    config = {
        'superpoint': {
    ...
    for i, info in enumerate(homo_pairs):
        ...
        if do_eval:
            ...
            est_homo_deg = pydegensac.findHomography(mkpts0, mkpts1)[0]
            ...
            corner_points_deg = cv2.perspectiveTransform(np.reshape(corner_points, (-1, 1, 2)), est_homo_deg).squeeze(1)
            ...
            error_deg = compute_pixel_error(corner_points_deg, corner_points_gt)
        ...
        if do_viz:
            ...
            m_thresh = matching.NCTR.config['match_threshold']
            small_text = [
    ...
    # Collate the results into a final table and print to terminal.
        ...
        print("For ransac results...")
        print('AUC@5\t AUC@10\t AUC@25\t Prec\t Recall\t f1-score\t')
        print('{:.2f}\t {:.2f}\t {:.2f}\t {:.2f}\t {:.2f}\t {:.2f}\t'.format(
            aucs_ransac[0], aucs_ransac[1], aucs_ransac[2], prec, rec, f1_score))
        print("For degensac results...")
        print('AUC@5\t AUC@10\t AUC@25\t Prec\t Recall\t f1-score\t')
        print('{:.2f}\t {:.2f}\t {:.2f}\t {:.2f}\t {:.2f}\t {:.2f}\t'.format(
            aucs_deg[0], aucs_deg[1], aucs_deg[2], prec, rec, f1_score))
